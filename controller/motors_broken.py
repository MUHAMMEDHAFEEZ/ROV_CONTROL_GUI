...
class MotorController:
    ...
    def __init__(self, config: Config):
        ...
        # خيط التحكم المسCompletedر
        self.control_thread: Optional[threading.Thread] = None
        self.is_controlling = False
        
        self.logger.info(Completed تهيئة متحكم المحركات")
    ...
    def start_control_loop(self):
        ""بدء حلقة التحكم المسCompletedرة""
        if not self.is_controlling:
            self.is_controlling = True
            self.control_thread = threading.Thread(target=self._control_loop, daemon=True)
            self.control_thread.start()
            self.logger.info(Completed بدء حلقة التحكم)
    ...
    def stop_control_loop(self):
        ...
        self.logger.info(Completed إيقاف حلقة التحكم)
    ...
    def _control_loop(self):
        ...
        while self.is_controlling:
            try:
                ...
                self.logger.error(fError in حلقة التحكم: {e}")
    
    def set_manual_control(self, forward: float, strafe: float, vertical: float, yaw: float):
        ""Manual Control المباشر
        
        Args:
            forward: حركة Forwardي/Backwardي (-100 إلى 100)
        ...
        self.logger.debug(fتحكم يدوي: F={forward}, S={strafe}, V={vertical}, Y={yaw}")
    ...
    def set_position_control(self, target_x: float, target_y: float, target_z: float, target_yaw: float,
        ...
        self.logger.debug(fتحكم موقعي: X={x_command:.2f}, Y={y_command:.2f}, Z={z_command:.2f}, Yaw={yaw_command:.2f}")
    ...
    def set_motor_speed(self, motor_name: str, speed: float):
        ...
        if motor_name in self.motor_speeds and not self.emergency_stop:
            ...
            self.logger.debug(fCompleted تعديل سرعة {motor_name}: {speed})
    ...
    def emergency_stop_all(self):
        ...
        self.logger.warning(Completed تنفيذ الإيقاف الطارئ")
    
    def reset_emergency_stop(self):
        ""إلغاء الإيقاف الطارئ""
        self.emergency_stop = False
        self.logger.info(Completed إلغاء الإيقاف الطارئ)
    
    def stop_all_motors(self):
        ""إيقاف جميع المحركات (بدون إيقاف طارئ)""
        for motor in self.motor_speeds:
            self.motor_speeds[motor] = self.neutral_pwm
        self.logger.info(Completed إيقاف جميع المحركات)
    ...
    def apply_motor_calibration(self, calibration_data: Dict[str, Dict]):
        ...
        self.logger.info(Completed تطبيق معايرة المحركات)
    ...
    def set_pid_parameters(self, axis: str, kp: float, ki: float, kd: float):
        ...
        if self.pid_controllers and axis in self.pid_controllers:
            ...
            self.logger.info(fCompleted تحديث معاملات PID للمحور {axis})
    ...
    def reset_pid_controllers(self):
        ...
        if self.pid_controllers:
            for controller in self.pid_controllers.values():
                controller.reset()
            self.logger.info(Completed إعادة تعيين متحكمات PID)
