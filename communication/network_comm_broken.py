...
class NetworkCommunication:
    ""فئة Connection الشبكي مع ROV (TCP/UDP)""
    ...
    def __init__(self, host: str = "192.168.1.100", port: int = 8080, protocol: str = "TCP):
        ...
        # خيط خادم UDP (للاسCompletedاع)
        self.server_thread: Optional[threading.Thread] = None
        self.is_server_running = False
    
    def connect(self) -> bool:
        "Connection بـ ROV عبر الشبكة""
        try:
            if self.protocol == "TCP":
                ...
                self.logger.error(fبروتوكول غير مدعوم: {self.protocol})
                return False
                
        except Exception as e:
            self.logger.error(fError in Connection الشبكي: {e})
            return False
    ...
    def _connect_tcp(self) -> bool:
        ...
        try:
            ...
            self.logger.info(fCompleted Connection TCP successfully إلى {self.host}:{self.port})
            ...
            self.logger.error(Completed رفض Connection)
            return False
        except Exception as e:
            self.logger.error(fError in اتصال TCP: {e})
            return False
    ...
    def _connect_udp(self) -> bool:
        ...
        try:
            ...
            self.logger.info(fCompleted إعداد اتصال UDP إلى {self.host}:{self.port})
            
            # بدء خادم UDP للاسCompletedاع
            self._start_udp_server()
            return True
            
        except Exception as e:
            self.logger.error(fError in اتصال UDP: {e})
            return False
    ...
    def disconnect(self):
        ...
        if self.socket_connection:
            ...
            self.logger.info(Completed قطع Connection الشبكي")
    
    def send_data(self, data: Dict[str, Any]) -> bool:
        ""إرسال Data إلى ROV""
        if not self.is_connected or not self.socket_connection:
            self.logger.warning(لا يوجد اتصال - لا يمكن إرسال Data)
            return False
        ...
        try:
            ...
            self.logger.debug(fCompleted إرسال Data: {data}")
            return True
            
        except Exception as e:
            self.logger.error(fError in إرسال Data: {e})
            return False
        ...
        while self.is_reading and self.is_connected:
            try:
                ...
                if message:
                    ...
                    self.logger.debug(fبيانات TCP واردة: {data_str})
                ...
                self.logger.error(fError in قراءة بيانات TCP: {e}")
                break
    
    def _udp_server(self):
        ""خادم UDP للاسCompletedاع للبيانات الواردة"
        listen_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        listen_socket.bind((, self.port + 1))  # استخدام منفذ مختلف للاسCompletedاع
        listen_socket.settimeout(1)
        
        self.logger.info(fبدء خادم UDP على المنفذ {self.port + 1}")
        ...
        while self.is_server_running:
            try:
                ...
                self.logger.debug(fبيانات UDP واردة من {addr}: {data_str})
                ...
                self.logger.error(fError in خادم UDP: {e})
                break
